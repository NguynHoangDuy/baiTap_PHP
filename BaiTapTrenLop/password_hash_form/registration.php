...
<?php
    ...
    {
        $i=0;
        while($rows = mysqli_fetch_array($resultEmail))
        {
            $listEmail[$i] = $rows["email"]; 
            $i++;
        }
    }
    function checkEmail($email, $listEmail){
    ...
    if(isset($_POST["submit"]))
    {
        ...
        $errEmail = checkEmail($email, $listEmail);
        if($err == "" && $name != "" && $email !="" && $errEmail == "") 
        {
            $option = ['cost'=> 12];
            $passHash = password_hash($pass, PASSWORD_DEFAULT, $option);
                                ...
                                <form class="mx-1 mx-md-4" method="post" action="">
                                    ...
                                    <div class="d-flex flex-row align-items-center mb-4" style="position: relative;">
                                        ...
                                        <div class="err-desc"><?php
                                            if(isset($errEmail)) echo "<p>$errEmail</p>"; echo "";?>
                                        </div>
                                    </div>
