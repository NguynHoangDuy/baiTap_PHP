...
<style>
    ...
    {
        $search = $_GET['search'];
        $loaiSua = $_GET['milkType'];
        $hangSua = $_GET['milkSup'];
                        ...
                        <?php
                                if(mysqli_num_rows($resultLoaiSua)!= 0)
                                {
                                    while($row = mysqli_fetch_array($resultLoaiSua))
                                    {
                                        ...
                                        if(isset($loaiSua))
                                        {
                                            if($loaiSua === $row[Ma_loai_sua])
                                            echo '<option selected value="'.$row[Ma_loai_sua].'">'.$row[Ten_loai].'</option>';
                                            else echo '<option value="'.$row[Ma_loai_sua].'">'.$row[Ten_loai].'</option>';
                                        }
                                        else {
                                ...
                                if(mysqli_num_rows($resultHangSua)!= 0)
                                {
                                    while($row = mysqli_fetch_array($resultHangSua))
                                    {
                                        ...
                                        if(isset($hangSua))
                                        {
                                            ...
                                            echo '<option selected value="'.$row[Ma_hang_sua].'">'.$row[Ten_hang_sua].'</option>';
                                            else echo '<option value="'.$row[Ma_hang_sua].'">'.$row[Ten_hang_sua].'</option>';
                                        }
                                        else {
    ...
    <?php
        ...
        if(!isset($_GET['search']) && !isset($_GET['milkType']) && !isset($_GET['milkSup']))
        {
            echo "<a href=".$_SERVER ['PHP_SELF']."?page=".$firstPage."> << </a>"; 
            echo "<a href=".$_SERVER ['PHP_SELF']."?page=".$prePage."> < </a>"; 
        } 
        else {
            echo "<a href=".$_SERVER ['PHP_SELF']."?milkType=".$loaiSua."&milkSup=".$hangSua."&search=".$search."&submit=&page=".$firstPage."> << </a>";   
            echo "<a href=".$_SERVER ['PHP_SELF']."?milkType=".$loaiSua."&milkSup=".$hangSua."&search=".$search."&submit=&page".$prePage."> < </a>";   
        }
        for($i = 1; $i <= $maxPage; $i++ ){
            if($i == $_GET['page'])
            echo '<b> '.$i.' </b>';
            else if(!isset($_GET['search']) && !isset($_GET['milkType']) && !isset($_GET['milkSup']))
            {
        ...
        if(!isset($_GET['search']) && !isset($_GET['milkType']) && !isset($_GET['milkSup']))
        {
            ...
            echo "<a href=".$_SERVER ['PHP_SELF']."?milkType=".$loaiSua."&milkSup=".$hangSua."&search=".$search."&submit=&page=".$nextPage."> > </a>";   
